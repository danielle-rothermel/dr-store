...
``LeaseAuthority.postgresql`` accepts a DSN and opens raw psycopg connections
per transaction. This is deliberate: authority transactions are independent
authoritative commits and must never enlist in a caller's evidence
transaction.
"""
...
_POSTGRES_TABLE_CONSTRAINTS: tuple[_PostgreSQLConstraint, ...] = (
    (
        _TABLE_NAME,
        "c",
...
_POSTGRES_METADATA_CONSTRAINTS: tuple[_PostgreSQLConstraint, ...] = (
    (
        ...
        *,
        ...
        if connect is None:
            try:
                ...
                raise ImportError(
                    "PostgreSQL lease authority requires psycopg"
                ) from exc
